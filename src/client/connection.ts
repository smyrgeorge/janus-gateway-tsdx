...
class Connection extends TTransactionGateway {
  ...
  constructor(id: string, address: string, options: ConnectionOptions,
              mediaDevices: MediaDevices, webRTC: WebRTC) {
    super()
  ...
  close(): Promise<any> {
    if (this.websocketConnection.isOpened()) {
      return Promise
        .map(this.getSessionList(), session => session.cleanup())
  ...
  toString() {
    return `JanusConnection${JSON.stringify({id: this.id, address: this.address})}`
  }

  private _onCreate(outMsg: JanusMessage): Promise<JanusMessage> {
    this.addTransaction(new Transaction(outMsg['transaction'], (msg: JanusMessage) => {
  ...
  private initWebsocketListeners() {
    ...
    this.websocketConnection.on('message', msg => {
      this.processIncomeMessage(new JanusMessage(msg))
