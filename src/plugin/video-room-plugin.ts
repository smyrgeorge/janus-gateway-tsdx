...
class VideoRoomPlugin extends MediaPlugin {
  ...
  start(room, jsep) {
    return this.sendWithTransaction({janus: 'message', body: {request: 'start', room}, jsep})
  }

  unpublish() {
    return this.sendWithTransaction({janus: 'message', body: {request: 'unpublish'}})
      .then(response => {
    ...
    return super.processIncomeMessage(message).then(result => {
      ...
      switch (type) {
        case 'event':
          ...
          this.handleRemotePublishers(pluginData.publishers)
            .then(() => console.debug('Success handle publishers.'))
