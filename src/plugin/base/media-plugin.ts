...
class MediaPlugin extends Plugin {
  ...
  createPeerConnection(options: RTCConfiguration = {}): RTCPeerConnection {
    let opts = Object.assign(options, this.getSession()?.getConnection()?.getOptions().pc ?? {})
  ...
  createAnswer(jsep: RTCSessionDescription, options: RTCAnswerOptions): Promise<any> {
    return Promise.try(() => this.setRemoteSDP(jsep))
      .then(() => this._createSDP('createAnswer', options))
  }
  ...
  processIncomeMessage(message: JanusMessage) {
    return Promise.try(() => super.processIncomeMessage(message))
      .then(result => {
